...
func TestTryLock(t *testing.T) {
	...
	unlockFn, err := locker.TryLock(repo, 1, workspace, projectName)
	Ok(t, err)

	// Now another lock for the same repo, workspace, and pull should fail
	_, err = locker.TryLock(repo, 1, workspace, projectName)
...
func TestTryLockDifferentWorkspaces(t *testing.T) {
	...
	_, err := locker.TryLock(repo, 1, workspace, projectName)
	Ok(t, err)
	_, err = locker.TryLock(repo, 1, "new-workspace", projectName)
	Ok(t, err)
...
func TestTryLockDifferentRepo(t *testing.T) {
	...
	_, err = locker.TryLock(newRepo, 1, workspace, projectName)
	Ok(t, err)
...
func TestTryLockDifferentPulls(t *testing.T) {
	...
	Ok(t, err)
	newPull := 2
	_, err = locker.TryLock(repo, newPull, workspace, projectName)
	Ok(t, err)
...
func TestUnlockDifferentWorkspaces(t *testing.T) {
	...
	unlockFn1, err1 := locker.TryLock(repo, 1, workspace, projectName)
	Ok(t, err1)
	unlockFn2, err2 := locker.TryLock(repo, 1, "new-workspace", projectName)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
...
func TestUnlockDifferentRepos(t *testing.T) {
	...
	Ok(t, err1)
	newRepo := "owner/newrepo"
	unlockFn2, err2 := locker.TryLock(newRepo, 1, workspace, projectName)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
...
func TestUnlockDifferentPulls(t *testing.T) {
	...
	Ok(t, err1)
	newPull := 2
	unlockFn2, err2 := locker.TryLock(repo, newPull, workspace, projectName)
	Ok(t, err2)
	unlockFn1()
	unlockFn2()
...
func TestLockPull(t *testing.T) {
	...
	Assert(t, err != nil, "exp err")
	_, err = locker.TryLock("owner/repo", 1, "workspace", "project")
	Assert(t, err != nil, "exp err")

	// Lock for a different pull and workspace should succeed.
	_, err = locker.TryLockPull("owner/repo", 2)
	Ok(t, err)
	_, err = locker.TryLock("owner/repo", 3, "workspace", "project")
	Ok(t, err)
...
func TestLockPull_WorkspaceFirst(t *testing.T) {
	locker := events.NewDefaultWorkingDirLocker()
	unlock, err := locker.TryLock("owner/repo", 1, "workspace", "project")
	Ok(t, err)
